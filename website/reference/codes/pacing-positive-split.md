---
description: "PACING_POSITIVE_SPLIT: Second half slower than the first by more than 2% — what this chiptime insight code means and how to handle it."
---
...
**Second half slower than the first by more than 2%**
...
for ins in report.sessions[0].insights:
    if ins.code == "PACING_POSITIVE_SPLIT":
        print(ins.message, ins.evidence)
```

All codes are stable machine contract — grep logs for the string `PACING_POSITIVE_SPLIT`,
