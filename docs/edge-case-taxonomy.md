...
45. **Timer event stack**: `timer start/stop_all/pause`, nested/unbalanced events, missing final stop (crash), `stop_all` without start, redundant `stop_all` after the final stop (Wahoo shutdown), boundary events leaking across multisport session windows (Suunto). → Reconstruct timer state machine defensively; redundant events are no-ops with provenance, never phantom intervals; derive timer-time when session summary is missing/wrong.
