...
| Guard ban on comparator-less `.sort()` over numbers | F31 | The `Math.round` guard's natural sibling, but it belongs with the code it protects. (The `Date` half of this entry landed at F34, with the timestamp formatters.) | F36 (sort sites in semantics) |
| Promote the real Wahoo ELEMNT ROAM race file and the Venice Suunto multisport file into the private real-file corpus tier | F45 | Files live outside the repo; promotion is a maintainer action under the F19 PII policy (`promote_real.py`) | When the maintainer runs the F19 promotion flow on those two files |
| Audit `_session_events` boundary semantics (a shared-second boundary event lands in two sessions' windows; timers now tolerate it, but gap-classification evidence consumes the same slices) | F45 | Timer machine is now defensive against the leak; no observed gap misclassification from it; a window-rule change would regen multisport expected outputs | If a multisport file shows boundary events misattributed in gap evidence |
