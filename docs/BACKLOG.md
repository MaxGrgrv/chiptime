...
| Redundant trailing `stop_all` opens a phantom timer interval (taxonomy #45 sub-case) | F8 | Found 2026-08-22 in a real ELEMNT ROAM race file: the device's standard shutdown writes `stop` then `stop_all` at the same timestamp; with no interval open, the stop-without-start heuristic anchors the `stop_all` at the *first record*, doubling derived timer time (18,828 s → 37,658 s) and tainting analyze durations/load. Fix: apply the first-record heuristic only when no intervals exist yet; a trailing redundant stop is a no-op. Needs corpus case (stop→stop_all shutdown) + TS twin in lockstep. | Bug, not deferred scope — fix at next work cycle (task chip created 2026-08-22) |
