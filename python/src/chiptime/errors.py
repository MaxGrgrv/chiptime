...
ERROR_CODES: dict[str, str] = {
    ...
    "TRIM_NO_RECORDS": "The file has no record messages, so trimmed totals could not be "
    "recomputed; nothing was written.",
...
PROVENANCE_CODES: dict[str, str] = {
    ...
    "records were kept.",
    "TRIM_SUMMARIES_REBUILT": "Session and activity totals were recomputed from the records "
    "that survived a trim, so the file cannot carry stale summaries.",
    ...
    "TIMER_REDUNDANT_STOP": "Timer stop arrived with no interval open; ignored as"
    " redundant (device shutdown / multisport boundary pattern).",
    "TIMER_REDUNDANT_START": "Timer start with no timer time after it; ignored as"
    " redundant (multisport boundary pattern).",
